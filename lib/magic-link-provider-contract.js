...
function buildProviderContract(options = {}) {
  ...
  const waitingRoom = {
    waitingRoomUrl: `${origin}/sala/${encodeURIComponent(waitingRoomToken)}`,
    waitingRoomPath: `/sala/${encodeURIComponent(waitingRoomToken)}`,
  };
...
function buildShareMessage({ origin, waitingRoomToken, studentName }) {
  return [
    ...
    `Abre este enlace único y de un solo uso para entrar en tu sala de espera y activar la app en tu móvil: ${origin}/sala/${encodeURIComponent(waitingRoomToken)}`,
